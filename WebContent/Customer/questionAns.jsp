...
<body style="background: linear-gradient(to right, #d7d2cc 0%, #304352 100%)">
    ...
    <div class="container">
	    <div class="tabs">
	        <div class="tab active" onclick="openTab(event, 'tab1')">Search Questions By Keywords</div>
	        <div class="tab"     onclick="openTab(event, 'tab2')">View Previously Asked Questions</div>
	    </div>
	
	    <div id="tab1" class="tab-content active">
	        <form action="questionAns.jsp" method="post">
	            <input type="text" name="keyword" placeholder="Enter keyword to search" required>
	            <button type="submit"  name="searchType"  value="searchQuestions">Search</button>
	        </form>
	        ...
	        try {
	            ...
	            if ("searchQuestions".equals(searchType) && keyword != null) {  ApplicationDB db = new ApplicationDB(); Connection con = db.getConnection();
	                ...
	                String query = "SELECT m.message_id, m.content, m.reply, m.timestamp, e.first_name, e.last_name  FROM Messages m LEFT JOIN Support s ON"
			                  + " m.support_username = s.username LEFT JOIN Employee e ON s.username = e.username WHERE m.content LIKE ? OR m.reply LIKE ?";
	                	
	                PreparedStatement ps = con.prepareStatement(query); ps.setString(1, "%" + keyword + "%"); ps.setString(2, "%" + keyword + "%");
	                ...
	                while (rs.next()) { hasResults = true; out.print("<tr>"); out.print("<td>" + rs.getInt("message_id") + "</td>");
	  ...
	  out.print("<td style='word-wrap: break-word; text-align: justify;'>" + (rs.getString("reply") != null ? rs.getString("reply") : "No answer yet") + "</td>");
	                ...
	                if (!hasResults) {
	                	 out.print("<tr><td colspan='5'>No questions matched your search.</td></tr>");
	                }
	    ...
	    <div id="tab2" class="tab-content">
		    ...
		    try {
		        ...
		        String query = "SELECT m.message_id, m.content, m.reply, m.timestamp, e.first_name, e.last_name  FROM Messages m LEFT JOIN Support s ON "
		                       + "m.support_username = s.username LEFT JOIN Employee e ON s.username = e.username";
